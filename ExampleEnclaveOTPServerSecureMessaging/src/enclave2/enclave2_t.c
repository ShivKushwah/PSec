...
static sgx_status_t SGX_CDECL sgx_session_request_enclave2(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_session_request_enclave2_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_session_request_enclave2_t* ms = SGX_CAST(ms_session_request_enclave2_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	ms->ms_retval = session_request_enclave2(ms->ms_src_enclave_id, _in_dh_msg1, _in_session_id);
	if (_in_dh_msg1) {
...
static sgx_status_t SGX_CDECL sgx_exchange_report_enclave2(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_exchange_report_enclave2_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_exchange_report_enclave2_t* ms = SGX_CAST(ms_exchange_report_enclave2_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	ms->ms_retval = exchange_report_enclave2(ms->ms_src_enclave_id, _in_dh_msg2, _in_dh_msg3, ms->ms_session_id);
	if (_in_dh_msg3) {
...
static sgx_status_t SGX_CDECL sgx_generate_response_enclave2(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_generate_response_enclave2_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_generate_response_enclave2_t* ms = SGX_CAST(ms_generate_response_enclave2_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	ms->ms_retval = generate_response_enclave2(ms->ms_src_enclave_id, _in_req_message, _tmp_req_message_size, ms->ms_max_payload_size, _in_resp_message, _tmp_resp_message_size);
	if (_in_resp_message) {
...
static sgx_status_t SGX_CDECL sgx_end_session_enclave2(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_end_session_enclave2_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_end_session_enclave2_t* ms = SGX_CAST(ms_end_session_enclave2_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = end_session_enclave2(ms->ms_src_enclave_id);


	return status;
}

static sgx_status_t SGX_CDECL sgx_test_create_session_enclave2(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_test_create_session_enclave2_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_test_create_session_enclave2_t* ms = SGX_CAST(ms_test_create_session_enclave2_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = test_create_session_enclave2(ms->ms_src_enclave_id, ms->ms_dest_enclave_id);


	return status;
}

static sgx_status_t SGX_CDECL sgx_test_enclave_to_enclave_call_enclave2(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_test_enclave_to_enclave_call_enclave2_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_test_enclave_to_enclave_call_enclave2_t* ms = SGX_CAST(ms_test_enclave_to_enclave_call_enclave2_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = test_enclave_to_enclave_call_enclave2(ms->ms_src_enclave_id, ms->ms_dest_enclave_id);


	return status;
}

static sgx_status_t SGX_CDECL sgx_test_message_exchange_enclave2(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_test_message_exchange_enclave2_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_test_message_exchange_enclave2_t* ms = SGX_CAST(ms_test_message_exchange_enclave2_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	ms->ms_retval = test_message_exchange_enclave2(ms->ms_src_enclave_id, ms->ms_dest_enclave_id, _in_message, ms->ms_messagelen);
...
static sgx_status_t SGX_CDECL sgx_test_close_session_enclave2(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_test_close_session_enclave2_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_test_close_session_enclave2_t* ms = SGX_CAST(ms_test_close_session_enclave2_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = test_close_session_enclave2(ms->ms_src_enclave_id, ms->ms_dest_enclave_id);
...
SGX_EXTERNC const struct {
	...
	9,
	{
		...
		{(void*)(uintptr_t)sgx_end_session_enclave2, 0},
		{(void*)(uintptr_t)sgx_test_create_session_enclave2, 0},
		{(void*)(uintptr_t)sgx_test_enclave_to_enclave_call_enclave2, 0},
		{(void*)(uintptr_t)sgx_test_message_exchange_enclave2, 0},
		{(void*)(uintptr_t)sgx_test_close_session_enclave2, 0},
	}
};
